...
vec2      color_texture_sz=vec2(480.0,270.0);					  // size of color_texture
vec2      color_texture_pow2_sz=vec2(480.0,270.0);		  // size of color texture rounded up to power of 2
vec2      screen_texture_sz=vec2(1920.0,1080.0);				// size of output resolution
vec2      screen_texture_pow2_sz=vec2(1920.0,1080.0);   // size of output resolution rounded up to power of 2

varying vec2 fragmentTextureCoordinates;
varying vec4 colourVarying;

//CRT Filter Variables
const float hardScan=-12.0; //-8,-12,-16, etc to make scalines more prominent.
const vec2 warp=vec2(1.0/48.0,1.0/64.0);  //adjusts the warp filter (curvature).
const float maskDark=0.2; //Sets how dark a "dark subpixel" is in the aperture pattern.
const float maskLight=1.25; //Sets how dark a "bright subpixel" is in the aperture pattern.
...
float Scan(vec2 pos,float off){
  ...
  return Gaus(dst+off,hardScan/(dot(col,col)*0.25+1.0));
#endif
}
...
void main(void)
{
  ...
  gl_FragColor.rgb=Tri(fragmentTextureCoordinates)*mix(vec3(1.0),Mask(col.xy),maskStrength);

#ifdef BLOOM
	gl_FragColor.rgb+=Bloom(fragmentTextureCoordinates)*bloomAmount; 
#endif
